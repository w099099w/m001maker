function uploadMultiple(file, type, makerInfo, callback) {
        ...
        if (file.size > burstSize) {
            ...
            if (generateID.result.url) {
                ...
                for (var i = 0; i < chunkArr.length; i++) {
                    ...
                    // 分片上传
                    let uploadChunk = await http.sendFileChunk(formData_upload)
            ...
            let affirm = await http.sendFile(formData_upload)
            loading.close();
            if (affirm.code === 0) {
